...
- [1. 前言](#1-前言)
- [2. Java 内存结构详解](#2-java-内存结构详解)
    - [2.1. 一 概述](#21-一-概述)
    - [2.2. 二 运行时数据区域](#22-二-运行时数据区域)
        - [2.2.1. 程序计数器](#221-程序计数器)
        - [2.2.2. Java 虚拟机栈](#222-java-虚拟机栈)
        - [2.2.3. 本地方法栈](#223-本地方法栈)
        - [2.2.4. 堆](#224-堆)
        - [2.2.5. 方法区](#225-方法区)
            - [2.2.5.1. 方法区和永久代的关系](#2251-方法区和永久代的关系)
            - [2.2.5.3. 为什么要将永久代 (PermGen) 替换为元空间 (MetaSpace) 呢?](#2253-为什么要将永久代-permgen-替换为元空间-metaspace-呢)
        - [2.2.6. 运行时常量池](#226-运行时常量池)
        - [2.2.7. 直接内存](#227-直接内存)
    - [2.3. 三 HotSpot 虚拟机对象探秘](#23-三-hotspot-虚拟机对象探秘)
        - [2.3.1. 对象的创建](#231-对象的创建)
            - [2.3.1.1. Step1:类加载检查](#2311-step1类加载检查)
            - [2.3.1.2. Step2:分配内存](#2312-step2分配内存)
            - [2.3.1.3. Step3:初始化零值](#2313-step3初始化零值)
            - [2.3.1.4. Step4:设置对象头](#2314-step4设置对象头)
            - [2.3.1.5. Step5:执行 init 方法](#2315-step5执行-init-方法)
        - [2.3.2. 对象的内存布局](#232-对象的内存布局)
        - [2.3.3. 对象的访问定位](#233-对象的访问定位)
    - [2.4. 四  重点补充内容](#24-四--重点补充内容)
        - [2.4.1. String 类和常量池](#241-string-类和常量池)
        - [2.4.2. String s1 = new String("abc");这句话创建了几个字符串对象？](#242-string-s1--new-stringabc这句话创建了几个字符串对象)
        - [2.4.3. 8 种基本类型的包装类和常量池](#243-8-种基本类型的包装类和常量池)
    - [2.5. 参考](#25-参考)
...
* 好处：
  ...
  4. 多态  
* jvm、jdk、jre概念
  <div align="center"><img width="400" src="imgs/jvm概念.JPG"></div>

* 学习路线
  <div align="center"><img width="500" src="imgs/学习路线.png"></div>
...
<div align="center"><img width="600" src="imgs/JVM运行时数据区域.png"></div>


**JDK 1.8 ：**

<div align="center">  <img src="imgs/Java运行时数据区域JDK1.8.png" width="600px"/></div>
...
#### 2.2.5.3. 为什么要将永久代 (PermGen) 替换为元空间 (MetaSpace) 呢?
...
<div align="center">  <img src="imgs/常量池信息.jpg" width="450px"/></div>

既然运行时常量池是方法区的一部分，自然受到方法区内存的限制，当常量池无法再申请到内存时会抛出 OutOfMemoryError 错误。

**JDK1.7 及之后版本的 JVM 已经将运行时常量池从方法区中移了出来，在 Java 堆（Heap）中开辟了一块区域存放运行时常量池。** 

![地图](http://my-blog-to-use.oss-cn-beijing.aliyuncs.com/18-9-14/26038433.jpg)
...
### 2.4.2. String s1 = new String("abc");这句话创建了几个字符串对象？
